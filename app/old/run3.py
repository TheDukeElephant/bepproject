...
def calibrate_in_room_air():
    """
    Calibrates the sensor in the current room air (assumed to be around 400 ppm).
    """
    print("Calibrating sensor in current room air...")
    for _ in range(3):  # Retry up to 3 times
        response = send_command("G")  # Fresh air calibration command
        if response and response.startswith("G"):
            print("Sensor successfully calibrated in room air.")
            return
        else:
            print(f"Retrying room air calibration. Response: {response}")
    print("Failed to calibrate in room air after 3 attempts.")
